...
namespace DAZ_Installer.WinApp
{
    internal class DPExtractJob
    {
        ...
        private void Processor_ProcessError(object sender, DPProcessorErrorArgs e) => throw new NotImplementedException();
        ...
        private DPProductRecord? CreateRecords(DPArchive arc)
        {
            if (arc.Type == ArchiveType.Product || arc.Type == ArchiveType.Unknown)
                return null;
            ...
            foreach (DPFile file in arc.Contents.Values.Where(f => f.ExtractedToTarget))
            {
                successfulFiles.Add(file.RelativePathToContentFolder);
                if (file.RelativePathToContentFolder != null)
                {
                    foldersExtracted.Add(Path.GetDirectoryName(file.RelativePathToContentFolder));
                }
            }
            var workingExtractionRecord =
                new DPExtractionRecord(Path.GetFileName(arc.FileName), UserSettings.DestinationPath, successfulFiles.ToArray(), null,
                null, foldersExtracted.ToArray(), 0);

            if (arc.Type != ArchiveType.Bundle)
            {
                ...
                var author = arc.ProductInfo.Authors.Count != 0 ? arc.ProductInfo.Authors.First() : null;
                var workingProductRecord = new DPProductRecord(arc.ProductInfo.ProductName, arc.ProductInfo.Tags.ToArray(), author,
